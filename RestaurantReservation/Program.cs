    ...
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            while (true)
            {
                ...
                Console.WriteLine("13. Calculate the total revenue generated by a specific restaurant");
                Console.WriteLine("14. Find Customers With Large PartySize");

                Console.WriteLine("0. Exit");
                var choice = Console.ReadLine();
                Console.WriteLine();

                if (string.IsNullOrEmpty(choice) || !(int.Parse(choice) >= 0 && int.Parse(choice) <= 14))
                {
                ...
                if (int.Parse(choice) >= 6 && int.Parse(choice) <= 14)
                {
                    switch (choice)
                    {
                        case "6":
                        ...
                        case "12":
                            ...
                            break;
                        case "13":
                            var result = context.Restaurants.Select(x => new
                            {
                                RestaurantId = x.RestaurantId,
                                Name = x.Name,
                                TotalRevenue = context.CalculateTheTotalRevenueGeneratedByASpecificRestaurant(x.RestaurantId)

                            }).ToList();
                            //lits result
                            foreach (var item in result)
                            {
                                Console.WriteLine($"Restaurant ID: {item.RestaurantId}, Name: {item.Name}, Total Revenue: {item.TotalRevenue}");
                            }
                            break;
                        case "14":
                            Console.WriteLine("Enter party size in ID:");
                            var partySize = ReadID();
                            var customers = await FindCustomersWithLargePartySizeAsync(partySize);
                            await customers.Print("Customers With Large Party Size");
                            break;
                    }
                    continue;
                }
        ...
        public static async Task<List<CustomerReservation>> FindCustomersWithLargePartySizeAsync(int partySize)
        {
            var partySizeparam = new SqlParameter("@PartySize", System.Data.SqlDbType.Int)
            {
                Value = partySize
            };

            return context.CustomerReservations
                .FromSql($"Exec dbo.sp_FindCustomersWithLargePartySize {partySize}")
                .ToList();
        }
    }
}
